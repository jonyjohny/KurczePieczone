...
<div class="leading-tight">
    ...
    <x-lv-drop-down :dropDownWidth="64">
        <x-slot name="trigger">
            <x-lv-select-button>
                {{ __('sorting.sort_by') }}@if ($sortableByName = $sortableBy->flip()->get($sortBy))
                    :
        ...
        <x-lv-drop-down.header label="{{ __('sorting.sort_by') }}" />

        {{-- Each sortable item --}}
        @foreach ($sortableBy as $title => $column)
            <a href="#!" wire:click.prevent="sort('{{ $column }}')"
                title="{{ __('sorting.sort_by') }} {{ $title }} {{ __($sortOrder == 'asc' ? 'sorting.descending' : 'sorting.ascending') }}"
