<div>
    ...
    <x-jet-form-section submit="createApiToken">
        <x-slot name="title">
            {{ __('profiles.CreateAPIToken') }}
        </x-slot>

        <x-slot name="description">
            {{ __('profiles.CreateAPITokenInfo') }}
        </x-slot>

        <x-slot name="form">
            <!-- Token Name -->
            <div class="col-span-6 sm:col-span-4">
                <x-jet-label for="name" value="{{ __('profiles.TokenName') }}" />
            ...
            @if (Laravel\Jetstream\Jetstream::hasPermissions())
                <div class="col-span-6">
                    <x-jet-label for="permissions" value="{{ __('profiles.Permissions') }}" />
    ...
    @if ($this->user->tokens->isNotEmpty())
        ...
        <div class="mt-10 sm:mt-0">
            <x-jet-action-section>
                <x-slot name="title">
                    {{ __('profiles.ManageAPITokens') }}
                </x-slot>

                <x-slot name="description">
                    {{ __('profiles.ManageAPITokensInfo') }}
                </x-slot>
    ...
    <x-jet-dialog-modal wire:model="managingApiTokenPermissions">
        <x-slot name="title">
            {{ __('profiles.APITokenPermissions') }}
        </x-slot>
    ...
    <x-jet-confirmation-modal wire:model="confirmingApiTokenDeletion">
        <x-slot name="title">
            {{ __('profiles.DeleteAPIToken') }}
        </x-slot>

        <x-slot name="content">
            {{ __('profiles.DeleteAPITokenConf') }}
        </x-slot>
